...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6285755495437@c.us'
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 12) {
                        found === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh Etzz', message.id).then(() => {
        ...
        function isBadwordMsg(id){
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 12) { // 12x
                        kasar === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh Etzz!', message.id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        }).then(() => {
                            const cus = id
                            var found = false
	...
	if (isGroupMsg && AntiStickerSpam && !isGroupAdmins && !isAdmin && !isOwner){
        ...
        if(!isCmd && isKasar && isGroupMsg && isBadword && !isGroupAdmins) { 
            ...
            if(isBadwordMsg(serial)) return
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin Etzz!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Etzz!', id)
        ...
        case prefix+'banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Etzz!', id)
        ...
        case prefix+'ttp2':
            ...
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#ttp2 [ Teks ]*, contoh *#ttp2 Etzz*`, id)
...
case prefix+'toimg':
        ...
        case prefix+'blackpink':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#blackpink [ Teks ]*, contoh *#blackpink Etzz*`, id)
            tobz.reply(from, mess.wait, id)
        ...
        case prefix+'pornhub':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Tobz|Dev Etzz*`, id)
        ...
        case prefix+'glitch':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Tobz|Dev Etzz*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                ...
                tobz.sendFileFromUrl(from, `https://arugaz.my.id/api/textpro/glitchtext?text1=${glitch1}&text2=${glitch2}`, 'glitch.jpg', '', id)
                await limitAdd(serial)
            } else {
        ...
        case prefix+'daftar':  // NAMBAHIN NOMOR DI DATABASE
                ...
                const umurnye = argz[2]
                    ...
                    if(umurnye >= 40) return await tobz.reply(from, 'Kamu terlalu tua, kembali lagi ke masa muda untuk menggunakan Etzz', id)
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
            ...
            case prefix+'daftarulang':
                    if (!isAdmin) return tobz.reply(from, 'Command ini hanya dapat digunakan oleh admin Etzz', id)  
...
${desc}`)
        ...
        case prefix+'simi':
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Etzz!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
...
🌐️ *URL*: ${url}`
	...
	case prefix+'bitly':
        ...
        case prefix+'ytmp4':
            ...
            try {
                tobz.reply(from, mess.wait, id)
                const ytvh = await fetch(`http://arugaz.my.id/api/media/ytvideo?url=${args[1]}`)
  ...
  const judulagu = body.slice(6)
            if(!judulagu) return await tobz.reply(from, 'Masukkan judul lagu nya dik!', id)
                    scdl.search("tracks", judulagu, CLIENT_ID).then(res => {
                        ...
                        scdl.getInfo(urllagu, CLIENT_ID).then(info =>{
							...
							if (info.full_duration >= 900000) return tobz.reply(from, "Lagu lebih dari 15 menit tidak diperbolehkan!", id)
                           ...
                           tobz.sendTextWithMentions(from, captione)
                        })
                        ...
                        await sleep(15000)
                        await tobz.sendFile(from, './media/audio/audio.mp3', 'audio.mp3', id)
                        limitAdd(serial)
                        await sleep(30000)
        ...
        case prefix+'ytmp3':
            ...
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                    ...
                    tobz.reply(from, 'Fitur Sedang Dalam Perbaikan', message.id)
...
• *Link:* https://twitter.com/${username}`
        ...
        case prefix+'igstalk':
            ...
            console.log(istalk)
            try {
            const istalk2 = await axios.get('http://docs-jojo.herokuapp.com/api/stalk?username=' + istalk)
...
• *Nama:* ${full_name}
• *Deskripsi:* ${biography}
• *Pengikut:* ${edge_followed_by}
• *Mengikuti*: ${edge_follow}
• *Jumlah Postingan:* ${post_count}
• *Private:* ${has_blocked_viewer}
• *Link:* https://instagram.com/${istalk}`
            
            const pictk = await bent("buffer")(profile_pic_url_hd)
            const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
...
case prefix+'stext':
                    ...
                    if (!tk) return await tobz.reply(from, 'Text tidak boleh kosong', id)
                    if (args.length >= 6) return await tobz.reply(from, 'Ups. Maks 5 kata tod!', id)
                    const txt = await stext(tk)
                    tobz.sendImageAsSticker(from, txt)                       
                        limitAdd(serial)
                    break
        case prefix+'checkip':
            if(isReg(obj)) return
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'textmaker':
                ...
                break
case 'invite':
tobz.reply(from, `Maaf ${pushname}, chat owner untuk memasukkan bot`, id)
 tobz.sendContact(chatId, `6285755495437@c.us`)
tobz.reply(from, `itu nomor pacar,eh owner`, id)
tobz.sendStickerfromUrl(from, 'https://media4.giphy.com/media/f0CamfJcZABbrPI1us/giphy.gif')
...
break
        ...
        case prefix+'addbadword':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin Etzz!`, id)
        ...
        case prefix+'bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner Etzz`, id)
                bctxt = body.slice(4)
                txtbc = `*「 Etzz BROADCAST 」*\n\n${bctxt}`
        ...
        case prefix+'ownergroup':
            ...
            break
        case prefix+'otagall': // FOR OWNER & ADMIN Etzz
        case prefix+'omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner Etzz', id)
        ...
        case prefix+'okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin Etzz', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Etzz', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Etzz', id)
            await tobz.sendText(from,'Etzz DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan Etzz`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan Etzz`, id)
            try {
                ...
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan Etzz atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
        ...
        case prefix+'getses':
            ...
            tobz.sendFile(from, sesPic, 'session.png', 'Nih Tod', id)
            break
        case prefix+'Etzzadmin':
            let admn = `This is list of Etzz Admin\nTotal : ${adminNumber.length}\n`
            ...
            break
 case 'screen': {
                    if (!isOwner) return await tobz.reply(from, 'Fitur ini hanya dapat digunakan oleh admin bot')
                    const snap = await tobz.getSnapshot()
                    tobz.sendImage(from, snap, 'snapshot.png', 'Nih Pribadi!!!')
                }
                    break
        case prefix+'limit':
            if(isReg(obj)) return
            ...
            break
        case prefix+'gift': // Hanya Admin & Owner Etzz yang bisa gift Limit
            if (!isAdmin, !isOwner) return tobz.reply(from, `Maaf, perintah ini hanya dapat dilakukan oleh Admin Etzz!`, id)
...
• Limit: ${limitCount-updated.limit}`
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Etzz!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin Etzz!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Etzz!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin Etzz!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Etzz!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
...
case prefix+'ava':
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin Etzz!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin Etzz!`, id)
        ...
        // LIST MENU
        case prefix+'runtime':
            tobz.reply(from, `Etzz telah aktif selama :\n${cts}`, id)
            break
        case prefix+'menu':
        case prefix+'help':
        ...
        case prefix+'adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Etzz', id)
...
// By Gimenz
        case prefix+'snk':
            ...
            break
        default:
            //if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Etzz, Link Ada Di Bio atau Bisa Mengetik #Etzzgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
